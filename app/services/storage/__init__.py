...
class BaseStorage:
    async def save_upload(self, upload: UploadFile, folder: str) -> StoredFile:  # pragma: no cover - interface
        raise NotImplementedError

    async def save_content(self, filename: str, content: bytes, mime_type: str, folder: str) -> StoredFile:
        raise NotImplementedError
...
class LocalStorage(BaseStorage):
    ...
    async def save_upload(self, upload: UploadFile, folder: str) -> StoredFile:
        safe_name = Path(upload.filename or "upload").name
        key = f"{folder}/{uuid4().hex}_{safe_name}"
    ...
    async def save_content(self, filename: str, content: bytes, mime_type: str, folder: str) -> StoredFile:
        ...
        size = len(content)
        url = f"{self.base_url}/{key}" if self.base_url else None
        return StoredFile(path=key, url=url, size=size, mime_type=mime_type)
...
class S3Storage(BaseStorage):
    ...
    async def save_content(self, filename: str, content: bytes, mime_type: str, folder: str) -> StoredFile:
        ...
        try:
            ...
            raise StorageError("Failed to upload content to object storage") from exc

        url = self._build_url(key)
        return StoredFile(path=key, url=url, size=len(content), mime_type=mime_type)
