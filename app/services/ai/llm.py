...
def generate_json(prompt: str, model: str | None = None, files: list[dict] | None = None) -> str:
    """
    Generates JSON output using the configured LLM provider.
    """
    provider = settings.LLM_PROVIDER.lower()
    
    if provider == "gemini":
        # Gemini usually handles JSON prompts well without specific flags, 
        # or we could use response_mime_type="application/json" in newer API versions.
        # For now, rely on prompt instructions.
        return _generate_gemini(prompt, model, files)
    elif provider == "ollama":
        return _generate_ollama(prompt, model, format="json", files=files)
    else:
        return _generate_gemini(prompt, model, files)
