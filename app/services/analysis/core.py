...
from app.services.ai.extraction import extract_text_from_bytes
from app.services.ai.llm import generate_text
from app.services.ai.llm_utils import parse_llm_json
from app.services.ai.prompts import build_assignment_extraction_prompt
# Re-export scoring function to maintain API compatibility if used elsewhere, 
# or we can update usages. For now, let's keep it here but import it.
from app.services.analysis.scoring import score_assignment_understanding, AssignmentAnalysisError
...
async def analyze_assignment_structure(
    ...
    for file in assignment.files:
        try:
            ...
            extracted_texts.append(extract_text_from_bytes(data, file.filename, file.mime_type))
            if settings.LLM_PROVIDER == "gemini":
                mime = file.mime_type or "application/pdf"
                print(f"DEBUG: Adding file payload for Gemini: {file.filename} ({mime})")
                file_payloads.append({"data": data, "mime_type": mime})
